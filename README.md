...
- `apps/web`: React + TypeScript + Vite 기반 앱
- `apps/web/public/data`: 검증된 공개 출처, 지명, 사건 데이터
- `docs`: 데이터 정책, 출처 로그, 검증 감사 문서
...
앱은 기본적으로 `http://127.0.0.1:5173`에서 실행됩니다.
...
- 중국/동아시아 중심 기본 지도
- 검증 기준점 2개: 성도, 한중
- 형주 고성/강릉 기준점은 2026-07-07 재검증에서 공식 출처 접근 확인에 실패하여 공개 데이터에서 제외
- 전투/사건 레이어는 위치 검증 전까지 공개 앱에서 비활성화
- 연도 필터: 184, 190, 200, 208, 219, 234, 263
- 지명/사건 선택 시 출처, 신뢰도, 메모 표시

## 데이터 원칙

1. 모든 지도 요소는 `source_ids`를 가져야 합니다.
2. 위치가 확정적이지 않으면 `confidence`와 `accuracy_radius_m`에 함께 기록합니다.
...
6. 접근 불가하거나 교차검증되지 않은 출처는 공개 데이터의 직접 근거로 사용하지 않습니다.

현재 앱 화면에는 공개 가능한 공식 출처 기준점만 표시한다는 주의 문구를 표시합니다.

## 주요 참고

- 프로젝트 계획서: `C:\Users\cheong\Desktop\SangokGIS_project_plan.pdf`
- NGA GNS: https://geonames.nga.mil/geonames/GNSHome/index.html
- CHGIS V6: https://chgis.fas.harvard.edu/data/chgis/v6/
- Scripta Sinica: https://www1.ihp.sinica.edu.tw/en/Research/Groups/scriptaSinica
